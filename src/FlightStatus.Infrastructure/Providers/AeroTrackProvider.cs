...
/// <summary>
/// Deterministic stub for AeroTrack. AeroTrack provides full detail:
/// status, scheduled + actual times, terminal, gate, and delay reason.
///
/// Scenario map
...
///   A500 - AeroTrack responds, QuickFlight has no data
///   A700 - both respond, AeroTrack has the OLDER lastUpdatedUtc
///   A800 - AeroTrack returns an unmapped/garbage status string
...
public class AeroTrackProvider : IFlightStatusProvider
{
    ...
    public Task<ProviderFlightStatusData?> GetStatusAsync(string flightNumber, string date, CancellationToken cancellationToken)
    {
        ...
        return Task.FromResult<ProviderFlightStatusData?>(new ProviderFlightStatusData
        {
            ...
            ActualDepartureUtc = stub.ActualDepartureUtc,
            ScheduledArrivalUtc = stub.ScheduledArrivalUtc,
            ActualArrivalUtc = stub.ActualArrivalUtc,
            Terminal = stub.Terminal,
            Gate = stub.Gate,
            DelayReason = stub.DelayReason,
